...
@section('title', __('Delivery Orders Management'))

@section('content')

<div class="mb-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
    <div>
        <h2 class="text-2xl font-bold text-gray-800 dark:text-white mb-1">{{ __('Independent Delivery Orders') }}</h2>
        <p class="text-sm font-medium text-gray-500">{{ __('Follow up on parcel shipments between two points independently') }}</p>
    </div>
</div>
...
<div class="container-3d">
    ...
    <div class="p-6 border-b border-gray-100 dark:border-white/5 bg-gray-50/50 dark:bg-white/5">
        <form action="{{ route('admin.deliveries.index') }}" method="GET" x-data x-ref="filterForm">
            <div class="flex flex-col xl:flex-row gap-4 items-center justify-between">
                ...
                <div class="relative w-full xl:w-1/3">
                    ...
                    @if(request('search'))
                        <a href="{{ route('admin.deliveries.index', request()->except('search', 'page')) }}" class="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-red-500"><i class="fa-solid fa-times-circle"></i></a>
                    @endif
                </div>
                ...
                <div class="flex flex-col sm:flex-row w-full xl:w-auto gap-4 items-center">
                    ...
                    <div class="w-full sm:w-48 relative">
                        <select name="sort" @change="$refs.filterForm.submit()" class="w-full appearance-none bg-white dark:bg-black/30 border border-gray-200 dark:border-white/10 text-gray-600 dark:text-gray-300 text-sm font-bold rounded-xl px-4 py-2.5 pr-10 focus:outline-none focus:border-blue-500 transition">
        ...
        <table class="w-full text-right whitespace-nowrap">
            <thead>
                <tr class="bg-gray-50/50 dark:bg-white/5 text-gray-500 dark:text-gray-400 text-sm border-b border-gray-100 dark:border-white/5">
                    ...
                    <th class="px-6 py-4 font-bold text-right">{{ __('Date and Time') }}</th>
                    <th class="px-6 py-4 font-bold text-right">{{ __('Sender Customer') }}</th>
                    <th class="px-6 py-4 font-bold text-right">{{ __('Assigned Agent') }}</th>
                    <th class="px-6 py-4 font-bold text-right">{{ __('Cost') }}</th>
                    <th class="px-6 py-4 font-bold text-right">{{ __('Delivery Status') }}</th>
                ...
                <tr class="hover:bg-gray-50/80 dark:hover:bg-white/5 transition duration-150">
                    ...
                    <td class="px-6 py-4 text-right">
                        <div class="text-sm font-bold text-amber-600 bg-amber-50 dark:bg-amber-500/10 dark:text-amber-400 px-3 py-1 rounded inline-block">
                            {{ mb_substr($delivery->user->name ?? 'غير متوفر', 0, 15) }}...
                        </div>
                    </td>
                    ...
                    <td class="px-6 py-4 text-right">
                        @if($delivery->agent)
                            <div class="text-sm font-bold text-blue-600 bg-blue-50 dark:bg-blue-500/10 dark:text-blue-400 px-3 py-1 rounded inline-block">
                                {{ mb_substr($delivery->agent->name, 0, 15) }}...
                            </div>
                        @else
                    ...
                    <td class="px-6 py-4 text-right">
                        <div class="font-black text-gray-800 dark:text-gray-200 flex items-center gap-1">
                            {{ number_format($delivery->delivery_cost ?? 0, 2) }} <span class="text-[10px] font-bold text-gray-400">SAR</span>
                        </div>
                    </td>
                    ...
                    <td class="px-6 py-4 text-right">
                        @if($delivery->status == 'pending')
                            <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded bg-amber-50 text-amber-600 text-xs font-bold border border-amber-100">
                                <i class="fa-solid fa-search-location"></i> جاري البحث
                            </span>
                        @elseif($delivery->status == 'in_progress')
                            <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded bg-blue-50 text-blue-600 text-xs font-bold border border-blue-100">
                                <i class="fa-solid fa-truck-fast"></i> في الطريق
                            </span>
                        @elseif($delivery->status == 'completed')
                            <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded bg-emerald-50 text-emerald-600 text-xs font-bold border border-emerald-100">
                                <i class="fa-solid fa-check-double"></i> تم التوصيل
                            </span>
                        @elseif($delivery->status == 'cancelled')
                            <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded bg-red-50 text-red-600 text-xs font-bold border border-red-100">
                                <i class="fa-solid fa-xmark"></i> ملغي
                            </span>
                        @endif
                    </td>
                    ...
                    <td class="px-6 py-4 text-center">
                        <a href="{{ route('admin.deliveries.show', $delivery->id) }}" class="inline-flex items-center justify-center gap-2 px-3 h-8 rounded-lg bg-gray-50 hover:bg-blue-600 dark:bg-white/10 text-gray-600 hover:text-white dark:text-gray-300 font-bold text-xs transition border border-gray-200 dark:border-white/5 hover:border-transparent">
                            {{ __('Details') }}
                        </a>
                    </td>
                </tr>
                ...
                <tr>
                    <td colspan="7" class="px-6 py-16 text-center">
                        <div class="flex flex-col items-center justify-center">
                            ...
                            <h3 class="text-base font-bold text-gray-600 dark:text-gray-300">{{ __('No delivery orders found') }}</h3>
                            <p class="text-sm text-gray-500 mt-1">{{ __('Check filters or try another search with a correct parcel number') }}</p>
                        </div>
                    </td>
                </tr>
                @endforelse
